...
export const generatePDFReport = async (prediction, imageSrc) => {
  ...
  pdf.text('Leafora Report', pageWidth / 2, 20, { align: 'center' })
  ...
  // Get the disease class (handle both 'class' from new predictions and 'predicted_class' from history)
  const diseaseClass = prediction.class || prediction.predicted_class
  ...
  pdf.text(`Condition: ${diseaseClass.replace(/_/g, ' ')}`, 15, yPos)
  yPos += 7
  ...
  pdf.text('Leafora - AI-Powered Diagnosis', pageWidth / 2, pageHeight - 10, { align: 'center' })
